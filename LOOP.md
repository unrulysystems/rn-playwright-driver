...
- U10 bug bash round 1 (2026-09-05, fresh participant, HEAD fabcaaf, iOS `rnpd-clean-seam`, six tasks, severity floor major): verdict `findings`. Two major, same root cause -- the runner is the documented workflow but had no presence in the root README: RNPD-U10-01 `@unrulysystems/rn-playwright-driver-runner` owns the `rn-driver` bin that "Running E2E Tests" prescribes and appeared in no package-table row, install line, or `docs/` file, so the README's own path ends at `rn-driver: command not found`; RNPD-U10-02 `rn-driver.config.ts` is required (CLI exits 2 naming the searched-from directory) and was documented nowhere while the README claimed the footprint is one Metro line and a plugin list. Both verified independently before the fix (`grep -c` on README.md returned 0 for each) and fixed in 753b776. Tasks 2-5 passed: iOS e2e 62 passed / 4 skipped EXIT 0; the release export is byte-identical with and without the marker and carries 0 driver strings; the production prebuild's Podfile carries the four-module exclude with 0 `RNDriver` matches under `ios/` and no `exampleUITests` target; both failure paths fail closed with a named stage. The gate stays open until a round runs clean on the fixed README.
- U10 below-floor follow-ups (2026-09-05, non-blocking, from the same round): (a) `--dry-run` never resolves the device, so a bogus `--device` prints byte-identical output and exits 0 while help says "Print the resolved plan"; (b) the companions are `bun add -d` in the README but `dependencies` in the example app; (c) the bad-device error names the stage and the input but offers no candidate list; (d) `rn-driver.config.ts` `playwright.specs` pins 5 paths (66 tests) while `playwright.config.ts` `testDir` is `./e2e` (15 spec files), so `e2e/core`, `e2e/assertions`, `e2e/locators`, three `e2e/primitives` specs and `e2e/visual` never run in the documented lane; (e) the example app installs `globalThis.__RN_DRIVER_EXAMPLE__` unconditionally in `App.tsx` and it ships in the production export -- app-owned rather than driver code, so the footprint claim holds, but the reference consumer does not model the BRIEF's "no test code in the app" persona; (f) the example's own scripts still invoke `nub`, which the README never introduces. The participant also left `examples/basic-app/ios/` as a production prebuild (marker unset, no `exampleUITests`), so an iOS `--skip-build` run fails until the runner regenerates it.

- U9 re-verified on 00852f6 (2026-09-05): the discovery fix mutated the driver after U9 closed, so both live gates were re-run rather than inherited. Example iOS pair on `rnpd-clean-seam` (`/tmp/rnpd/ios-pair-u10c.log`): 62 passed / 4 skipped, twice. Example Android pair on `emulator-5554` (`/tmp/rnpd/android-pair-u10e.log`): 64 passed / 2 skipped, twice. A first Android attempt (`/tmp/rnpd/android-pair-u10d.log`) failed both runs at `metro.preflight` with `localhost:8083 is already in use` because the iOS pair was still holding that port -- the example's two lanes share `metro.port` 8083, so they cannot run concurrently on one host; the runner failed closed and named the port, the fix, and the config key.

- U10 bug bash round 2 (2026-09-06, fresh participant, HEAD `c738281`, iOS `rnpd-clean-seam`, six tasks, severity floor major): verdict `findings`, 6 of 6 tasks run in ~19 min. Tasks 2-6 all passed and were stronger than round 1: iOS e2e 62 passed / 4 skipped exit 0 three times, once from a fully regenerated marker-off native project; the release export's two marker states differ by exactly one byte-run, Expo's own random bundler temp path, with identical Hermes source hashes and 0 driver strings in the `.hbc`; the marker-off production prebuild pod-installed 246 pods with 0 `RNDriver` entries in `Podfile.lock` and no `*UITests` scaffold; deleting `.expo/rn-driver-e2e-entry.js` and running a marker-off `expo start` did not recreate it. `nub run check:footprint` exit 0, 17/17. Two majors, both **documentation contract drift, not behavior**:
  - **F1** root `README.md` Requirements said `Node >= 18` while every command it then prescribes runs through the runner, which declares `engines.node >= 22` (its readiness probes need a global `WebSocket`/`fetch`, REQ-CLI-008). A consumer meeting the stated requirement exactly could run none of the documented commands.
  - **F2** `--dry-run` accepts a `--device` that matches nothing and exits 0 with a byte-identical plan (verified: `diff` of good vs bogus differs only in a Node warning PID). The behavior is **correct per REQ-CLI-002**, which requires exit 0 with *no device touch* — that is what lets `--dry-run` work offline with no Xcode or Android SDK. Three docs had drifted from that contract (the flag table's "resolved plan", the pipeline diagram placing `resolve` before the plan `--dry-run` shows, root `README.md:372`) and `examples/basic-app/README.md` documents `--dry-run` as the pre-flight to run first, so a typo'd `--device` read as a passing check on a host with five booted simulators from different lanes. **Resolved as a docs fix plus an explicit notice, not a behavior change**: amending REQ-CLI-002 to resolve the device would destroy the offline property and is outside this campaign's declared targets (REQ-SEAM-*).
